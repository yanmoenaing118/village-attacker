...
export default class PathRect extends Entity {
  ...
  constructor(w: number, h: number, public styles?: Partial<Styles>, pos?: Vec2 ) {
    super();
    this.w = w;
    this.h = h;
  ...
  update(dt: number, t: number): void {
      if(game.mousePos){
        this.clicked = game.ctx.isPointInPath(this.path, game.mousePos.x, game.mousePos.y);
        console.log(JSON.stringify(game.mousePos))
      }
      if(this.clicked){
